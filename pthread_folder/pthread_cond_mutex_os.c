...
/*
当write线程抢到锁后，会去判断缓冲区是否已满，如果没满，则调用wait进入阻塞态并自动释放锁，此时read线程会去抢占锁并把字母读入缓冲区,read线程直到收到信号后自动上锁，继续运行while来判断缓冲区；如果缓冲区已满，则不会执行while循环里的wait函数，直接开始写数据，写完后清空缓冲区并解锁。如果又一次被write抢到了锁，之后步骤和上面一样。
*/
